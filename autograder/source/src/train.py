...
def main():
    ...
    if 'l1_size' in config:
        # Gather all 6 possible sizes into a master list
        master_sizes = [
        ...
        # If num_layers is 3, it only takes the first 3 sizes!
        actual_num_layers = config.num_layers
        actual_hidden_sizes = master_sizes[:actual_num_layers]
        
        # Overwrite args so the rest of your code works normally
        args.num_layers = actual_num_layers
        args.hidden_size = actual_hidden_sizes
        
        # Update config so W&B dashboard shows the clean list
        wandb.config.update({
